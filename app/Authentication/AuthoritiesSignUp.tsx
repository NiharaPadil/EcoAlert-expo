...
//     // Basic validation
//     if (!name || !phoneNum || !email || !organisation|| !password || !designation) {
...
//         PhoneNum: phoneNum,
//         Email: email,
//         Role: 'Authority', // Default role
//         AuthorityId: AuthorityCredential.user.uid, // Store Firebase UID
//         Password:password
//       });
...
const AuthoritiesSignup = () => {
  ...
  const SignInMainPage = () => {
    router.push('/Authentication/SignInMainPage'); // Navigate to sign-in page
  };
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      Alert.alert('Authority registered successfully!');
      router.push('/Authentication/SignInMainPage'); // Navigate to the sign-in page
    } catch (error) {
      console.error('Error registering Authority: ', error);
      Alert.alert('Error registering Authority. Please try again.');
    }
